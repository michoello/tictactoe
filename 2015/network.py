...
class Network():
    ...
    # ---------
    # my wrappers hiding numpy stuff
    
    def predict(self, x):
        _x = np.asarray(x).reshape((self.sizes[0],1))
        _y = self.feedforward(_x)
        return sum(_y.tolist(), [])

    def train(self, x, y, eta, epochs):
        _x = np.asarray(x).reshape((self.sizes[0],1))
        _y = np.asarray(y).reshape((self.sizes[-1],1))
        for _ in range(epochs):
            nabla_b = [np.zeros(b.shape) for b in self.biases]
            nabla_w = [np.zeros(w.shape) for w in self.weights]

            delta_nabla_b, delta_nabla_w = self.backprop(_x, _y)
...
if __name__ == "__main__":
    net = Network([72,36,16,4,2])
    x = [ 1 if random.random() > 0.5 else 0 for _ in range(72) ]
